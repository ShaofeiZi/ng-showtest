 ...
 */
angular.module('main')
    .controller('liebiaoCtrl', function ($scope, $http, $state) {
...
//计算器
        ...
        //全选
        $scope.xuanzhong = [];//定义数组用于存放前端显示
        var str = "";//
        var flag = '';//是否点击了全选，是为a
        $scope.xuan = true;//默认未选中
        ...
        $scope.all = function (c, v) {//全选
            ...
            if (c === false) {
                ...
                for (var i = 0; i < v.length; i++) {
                    $scope.xuanzhong[i] = '' + v[i].id + '';
                    //$scope.xuanzhong[i] = undefined;
                }
            }

            flag = 'a';

            console.log($scope.xuanzhong);
            //console.log(flag);
        };
        ...
        $scope.chk = function (z, x) {//单选或者多选
            ...
            $scope.xuanzhong = (str.substr(0, str.length - 1)).split(',');
            console.log($scope.xuanzhong);
        };


        $scope.deletes = function () {// 删起来啊

            if ($scope.xuanzhong.length == 0) {//没有选择一个的时候提示
                alert("请至少选中一条数据在操作！")
                return;
            }
            else {

                var id = $scope.xuanzhong;
                $http({
                    method: 'POST',
                    url: "/a/students/",
                    //url: "/a/students/?id="+ids,
        ...
        $scope.typearr = ["CSS", "JS", "JAVA", "运维", "DBA", "产品", "IOS", "android"];
        $http.get('/a/students', {})
        ...
        $scope.shanchu = function (num) {
            var id = $scope.data[(num)].id;
            $http({
                method: 'POST',
                url: "/a/students/",
                //url: "/a/students/?id="+ids,
